(function (global, factory) {
	...
	var gsap,
		...
		_setTouchActionForAllDescendants =
			function _setTouchActionForAllDescendants(elements, value) {
				...
				while (i--) {
					...
					children &&
						children.length &&
						_setTouchActionForAllDescendants(children, value);
				}
			},
		...
		_getMaxScroll = function _getMaxScroll(element, axis) {
			...
			return Math.max(
				0,
				_isRoot(element)
					? Math.max(_docElement$1[scroll], _body$1[scroll]) -
							(_win$1['inner' + dim] ||
								_docElement$1[client] ||
								_body$1[client])
					: element[scroll] - element[client]
			);
		},
		...
		_getComputedStyle = function _getComputedStyle(element) {
			return _win$1.getComputedStyle(
		...
		_parseRect = function _parseRect(e) {
			if (e === _win$1) {
				...
				_tempRect.width = _tempRect.right =
					_docElement$1.clientWidth || e.innerWidth || _body$1.clientWidth || 0;
				_tempRect.height = _tempRect.bottom =
					(e.innerHeight || 0) - 20 < _docElement$1.clientHeight
						? _docElement$1.clientHeight
						: e.innerHeight || _body$1.clientHeight || 0;
				return _tempRect;
			}
			...
			var doc = e.ownerDocument || _doc$1,
				r = !_isUndefined(e.pageX)
					...
					: !e.nodeType && !_isUndefined(e.left) && !_isUndefined(e.top)
					? e
		...
		_getElementBounds = function _getElementBounds(element, context) {
			...
			matrix = getGlobalMatrix(context, true).multiply(
				getGlobalMatrix(element)
			);
			p1 = matrix.apply({
				x: left,
				y: top,
		...
		_parseInertia = function _parseInertia(
			draggable,
			snap,
			max,
			min,
		...
		_isClickable = function _isClickable(element) {
			var data;
			return !element || !element.getAttribute || element === _body$1
				? false
				: (data = element.getAttribute('data-clickable')) === 'true' ||
				  (data !== 'false' &&
						(_clickableTagExp.test(element.nodeName + '') ||
							element.getAttribute('contentEditable') === 'true'))
				? true
				: _isClickable(element.parentNode);
		},
		...
		_initCore = function _initCore(required) {
			if (_windowExists() && document.body) {
				...
				_isAndroid =
					nav && nav.userAgent.toLowerCase().indexOf('android') !== -1;
				_isTouchDevice =
					('ontouchstart' in _docElement$1 && 'orientation' in _win$1) ||
				...
				_touchEventLookup = (function (types) {
					var standard = types.split(','),
						converted = (
							'onpointerdown' in _tempDiv
								? 'pointerdown,pointermove,pointerup,pointercancel'
								: 'onmspointerdown' in _tempDiv
					...
					try {
						_docElement$1.addEventListener(
							'test',
							null,
	...
	var Draggable = (function (_EventDispatcher) {
		...
		function Draggable(target, vars) {
			...
			_this2.styles =
				_getStyleSaver && _getStyleSaver(target, 'transform,left,top');
			...
			var type = (vars.type || 'x,y').toLowerCase(),
						...
						while (e && !isRoot) {
							...
							rect = isRoot
								? {
										bottom: Math.max(
											_docElement$1.clientHeight,
											_win$1.innerHeight || 0
										),
										right: Math.max(
											_docElement$1.clientWidth,
											_win$1.innerWidth || 0
										),
										left: 0,
										top: 0,
							...
							if (allowY) {
								...
								if (gap < 0) {
									...
									pointerY > rect.bottom - autoScrollMarginBottom &&
									gap
								) {
									...
									changeY = Math.min(
										...
										(autoScrollFactor *
											(1 -
												Math.max(0, rect.bottom - pointerY) /
													autoScrollMarginBottom)) |
											0
									);
								} else if (
									pointerY < rect.top + autoScrollMarginTop &&
									parent.scrollTop
								) {
									checkAutoScrollBounds = true;
									changeY = -Math.min(
							...
							if (allowX) {
								...
								if (gap < 0) {
									...
									pointerX > rect.right - autoScrollMarginRight &&
									gap
								) {
									checkAutoScrollBounds = true;
									changeX = Math.min(
										...
										(autoScrollFactor *
											(1 -
												Math.max(0, rect.right - pointerX) /
													autoScrollMarginRight)) |
											0
									);
								} else if (
									pointerX < rect.left + autoScrollMarginLeft &&
									parent.scrollLeft
								) {
									checkAutoScrollBounds = true;
									changeX = -Math.min(
				...
				syncXY = function syncXY(skipOnUpdate, skipSnap) {
					...
					if (xyMode) {
						...
						self.y =
							parseFloat(
								target.style.top || ((cs = _getComputedStyle(target)) && cs.top)
							) || 0;
						...
						return function (n) {
							...
							return (
								snap.call(
									self,
									(n > max
										? max + (n - max) * edgeTolerance
										: n < min
										? min + (n - min) * edgeTolerance
										: n) * factor
								) * factor
							);
						};
				...
				buildPointSnapFunc = function buildPointSnapFunc(
					snap,
					minX,
					maxX,
					minY,
					...
					if (_isFunction(snap)) {
						return function (point) {
							...
							point.x = x =
								x > maxX
									? maxX + (x - maxX) * edgeTolerance
									: x < minX
									? minX + (x - minX) * edgeTolerance
									: x;
							point.y = y =
								y > maxY
									? maxY + (y - maxY) * edgeTolerance
									: y < minY
									? minY + (y - minY) * edgeTolerance
									: y;
				...
				calculateBounds = function calculateBounds() {
					...
					if (scrollProxy) {
						...
						if (rotationMode) {
							...
							!_isUndefined(vars.bounds.maxX) ||
							!_isUndefined(vars.bounds.maxY)
						) {
							bounds = vars.bounds;
							...
							self.minX = minX = Math.round(
								getPropAsNum(xProp, 'px') + bounds.left - targetBounds.left
							);
							self.minY = minY = Math.round(
								getPropAsNum(yProp, 'px') + bounds.top - targetBounds.top
							);
					...
					if (vars.liveSnap) {
						...
						if (rotationMode) {
							...
							if (snap.points) {
								...
								if (allowX) {
									snapX = buildSnapFunc(
										snapIsRaw ? snap : snap.x || snap.left || snap.scrollLeft,
										minX,
										maxX,
										scrollProxy ? -1 : 1
									);
								...
								if (allowY) {
									snapY = buildSnapFunc(
										snapIsRaw ? snap : snap.y || snap.top || snap.scrollTop,
										minY,
										maxY,
										scrollProxy ? -1 : 1
									);
				...
				animate = function animate(inertia, forceZeroVelocity) {
					...
					if (inertia && InertiaPlugin) {
						if (inertia === true) {
							...
							inertia = {
								resistance:
									(vars.throwResistance || vars.resistance || 1000) /
									(rotationMode ? 10 : 1),
							};
							...
							if (rotationMode) {
								...
								if (allowX) {
									inertia[xProp] = _parseInertia(
										self,
										snapIsRaw ? snap : snap.points || snap.x || snap.left,
										maxX,
										minX,
										scrollProxy ? -1 : 1,
								...
								if (allowY) {
									inertia[yProp] = _parseInertia(
										self,
										snapIsRaw ? snap : snap.points || snap.y || snap.top,
										maxY,
										minY,
										scrollProxy ? -1 : 1,
							...
							inertia.duration = {
								...
								min: !isNaN(vars.minDuration)
									...
									: overshootTolerance === 0 ||
									  (_isObject(inertia) && inertia.resistance > 1000)
									? 0
									: 0.5,
				...
				updateMatrix = function updateMatrix(shiftStart) {
					...
					if (
						shiftStart &&
						self.isPressed &&
						!matrix.equals(start || new Matrix2D())
					) {
					...
					if (scrollProxy) {
						...
						if (rotationMode) {
							parsedOrigin = target.ownerSVGElement
								...
								: (
										_getComputedStyle(target)[_transformOriginProp$1] || '0 0'
								  ).split(' ');
					...
					if (touchEventTarget && allowNativeTouchScrolling && !touchDragAxis) {
						...
						if (
							...
							if (
								...
								_isFunction(self.vars.onLockAxis) &&
				...
				setPointerPosition = function setPointerPosition(
					pointerX,
					pointerY,
					invokeOnMove
				) {
