(function (global, factory) {
	...
	const _complexStringNumExp = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi;
	const _relExp = /[+-]=-?[.\d]+/;
	...
	const _warn = (message, suppress) => !suppress && console.warn(message);
	const _addGlobal = (name, obj) =>
		(name &&
			(_globals[name] = obj) &&
			_installScope &&
	...
	const _getCache = (target) => {
		return target._gsap || _harness(toArray(target))[0]._gsap;
	};
	const _getProperty = (target, property, v) => {
	...
	const _forEachName = (names, func) => {
		return (names = names.split(',')).forEach(func) || names;
	};
	const _round = (value) => {
		return Math.round(value * 100000) / 100000 || 0;
	};
	const _roundPrecise = (value) => {
		return Math.round(value * 10000000) / 10000000 || 0;
	};
	const _parseRelative = (start, value) => {
	...
	const _arrayContainsAny = (toSearch, toFind) => {
		var l = toFind.length,
			i = 0;
	...
	const _lazySafeRender = (animation, time, suppressEvents, force) => {
	...
	const _numericIfPossible = (value) => {
	...
	const _passThrough = (p) => {
		return p;
	};
	const _setDefaults = (obj, defaults) => {
	...
	_setKeyframeDefaults = (excludeDuration) => {
	...
	const _copyExcluding = (obj, excluding) => {
		var copy = {},
			p;
